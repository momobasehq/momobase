...
import { MomobaseClient, type AvailablePaymentMethod, type ServiceType } from "@momobase/sdk"
...
export function AppTester({ clientId, clientSecret }: AppTesterProps) {
  const [credentials, setCredentials] = useState({ clientId: clientId ?? "", clientSecret: clientSecret ?? "" })
  ...
  async function run(label: string, action: (client: MomobaseClient) => Promise<unknown>) {
    if (!client) {
  ...
  async function pay(service: ServiceType) {
    ...
    await run(service === "collection" ? "Collection" : "Disbursement", async (app) => {
      const created =
        service === "collection"
          ? await app.collections.create({ ...payload, customer: { name: "Dashboard test" } }, { idempotencyKey: own })
          : await app.disbursements.create({ ...payload, recipient: { name: "Dashboard test" } }, { idempotencyKey: own })
      // Carry the identifiers straight into the lookup tab; copying them by hand is the
      // step that makes a tester tedious.
      setLookup({ id: created.transaction_id, reference: created.reference })
      return created
    })
  }
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          Runs the public API as the application, over the same token grant and scopes an integrator uses. Payments
          created here are real.
        </CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="tester-client-secret">Client secret</Label>
            <Input
        ...
        <Tabs defaultValue="auth">
          ...
          <TabsContent value="auth" className="flex flex-col gap-3 pt-3">
            <p className="text-muted-foreground">
              Exchanges the credentials for an access token, which is the first thing an integration does and the
              quickest way to tell a bad secret from a bad scope.
            </p>
            <div className="flex flex-wrap gap-2">
              <Button disabled={!ready || Boolean(busy)} onClick={() => void run("Token", (app) => app.authenticate())}>
                {busy === "Token" ? <Loader2 className="animate-spin" /> : <KeyRound />}
                Get a token
              </Button>
              <Button
                variant="outline"
                ...
                onClick={() =>
                  void run("Payment methods", async (app) => {
                    const available = await app.paymentMethods.list()
                    setMethods(available.items)
                    // Adopt the first method so the payment form is usable immediately.
                    if (available.items[0] && !form.payment_method) {
                      setForm((current) => ({ ...current, payment_method: available.items[0]!.payment_method }))
                    }
                    return available
                  })
                }
          ...
          <TabsContent value="pay" className="flex flex-col gap-3 pt-3">
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="flex flex-col gap-2">
                <Label htmlFor="tester-method">Payment method</Label>
                ...
                <datalist id="tester-methods">
                  {[...new Set(methods.map((method) => method.payment_method))].map((method) => (
              ...
              <div className="flex flex-col gap-2">
                <Label htmlFor="tester-amount">Amount (minor units)</Label>
                <Input
                  id="tester-amount"
                  type="number"
                  min={1}
              ...
              <div className="flex flex-col gap-2">
                <Label htmlFor="tester-currency">Currency</Label>
                <Input
                  id="tester-currency"
                  maxLength={3}
                  value={form.currency}
            ...
            <div className="flex flex-wrap gap-2">
              <Button disabled={!ready || Boolean(busy) || !form.payment_method} onClick={() => void pay("collection")}>
                {busy === "Collection" ? <Loader2 className="animate-spin" /> : <Send />}
                Collect
              </Button>
              <Button
                variant="outline"
                disabled={!ready || Boolean(busy) || !form.payment_method}
                onClick={() => void pay("disbursement")}
              >
                {busy === "Disbursement" ? <Loader2 className="animate-spin" /> : <Send />}
                Disburse
              </Button>
            </div>
          </TabsContent>

          <TabsContent value="lookup" className="flex flex-col gap-3 pt-3">
            <p className="text-muted-foreground">
              A payment created above fills these in, so its status can be read back through the same app session.
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="flex gap-2">
                <Input
                  className="font-mono"
                  value={lookup.id}
                  onChange={(event) => setLookup({ ...lookup, id: event.target.value })}
                  placeholder="Transaction ID"
                  aria-label="Transaction ID"
                />
                <Button
                  variant="outline"
                  disabled={!ready || Boolean(busy) || !lookup.id}
                  onClick={() => void run("Transaction", (app) => app.transactions.get(lookup.id))}
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  onChange={(event) => setLookup({ ...lookup, reference: event.target.value })}
                  placeholder="Your reference"
                  aria-label="Reference"
                />
                <Button
                  variant="outline"
                  disabled={!ready || Boolean(busy) || !lookup.reference}
                  onClick={() => void run("By reference", (app) => app.transactions.getByReference(lookup.reference))}
        ...
        {result && (
          <div className="flex flex-col gap-2">
            ...
            <pre className="bg-muted max-h-72 overflow-auto p-3 font-mono">{JSON.stringify(result.body, null, 2)}</pre>
          </div>
        )}
      </CardContent>
    </Card>
