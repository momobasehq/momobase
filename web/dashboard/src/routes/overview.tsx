...
function Tile({ label, value, hint, icon: Icon, loading }: {
  label: string
  value: string | number
  hint?: string
  ...
  return (
    <Card size="sm">
      <CardHeader>
        ...
        <CardTitle className="text-2xl">{loading ? <Skeleton className="h-7 w-16" /> : value}</CardTitle>
      </CardHeader>
      {hint && <CardContent className="text-muted-foreground">{hint}</CardContent>}
    </Card>
  )
}
    ...
    <div className="flex flex-col gap-4">
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <Tile
          ...
          hint={health.data ? `Database ${health.data.database}` : undefined}
        ...
        <Tile
          ...
          hint={health.data?.workers_configured.join(", ") || "None configured"}
          icon={Activity}
          loading={health.isPending}
        />
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>What this binary is running.</CardDescription>
        </CardHeader>
        <CardContent>
          {info.isPending ? (
            ...
            <dl className="grid gap-x-8 gap-y-3 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                ["Server time", formatDateTime(info.data?.server_time)],
              ].map(([label, value]) => (
                <div key={label}>
              ...
              <div>
                <dt className="text-muted-foreground">Workers enabled</dt>
                <dd className="mt-1">
                  <StatusBadge status={info.data?.workers_enabled ? "active" : "inactive"} />
                </dd>
              </div>
            </dl>
          )}
