...
export function ApiTester() {
  ...
  async function send() {
    ...
    try {
      ...
      setResult({ status: response.status, durationMs: Math.round(performance.now() - started), body: formatted })
    } catch (cause) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Issue a request and see the raw response envelope.</CardDescription>
      </CardHeader>
      <CardContent className="flex flex-col gap-4">
        <div className="flex flex-col gap-2 sm:flex-row">
          <Select value={method} onValueChange={(value) => setMethod((value as (typeof methods)[number]) ?? "GET")}>
            <SelectTrigger className="sm:w-32" aria-label="HTTP method">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              {methods.map((value) => (
          ...
          <Input value={path} onChange={(event) => setPath(event.target.value)} className="flex-1 font-mono" aria-label="Path" />
        ...
        {method !== "GET" && (
          <div className="flex flex-col gap-2">
            ...
            <Textarea id="tester-body" rows={6} className="font-mono" value={body} onChange={(event) => setBody(event.target.value)} />
          </div>
        )}
        ...
        {result && (
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3">
              <StatusBadge status={result.status < 400 ? "succeeded" : "failed"} />
              <span className="font-medium">{result.status}</span>
              <span className="text-muted-foreground">{result.durationMs} ms</span>
            </div>
            <pre className="bg-muted max-h-96 overflow-auto p-3 font-mono">{result.body}</pre>
          </div>
        )}
      </CardContent>
    </Card>
