import { Activity, AppWindow, FileClock, LayoutDashboard, Plug, Receipt, Route as RouteIcon, ShieldCheck, TerminalSquare, Users } from "lucide-react"
...
export const navigation: NavGroup[] = [
  {
    label: "Overview",
    items: [
      { to: "/", label: "Dashboard", icon: LayoutDashboard, permission: AdminPermissions.systemRead, end: true },
      { to: "/transactions", label: "Transactions", icon: Receipt, permission: AdminPermissions.transactionsRead },
    ],
  },
  {
    label: "Configuration",
    items: [
      { to: "/apps", label: "Apps", icon: AppWindow, permission: AdminPermissions.appsRead },
      { to: "/providers", label: "Providers", icon: Plug, permission: AdminPermissions.providersRead },
      { to: "/routes", label: "Routes", icon: RouteIcon, permission: AdminPermissions.routesRead },
    ],
  },
  {
    label: "Administration",
    items: [
      { to: "/users", label: "Users", icon: Users, permission: AdminPermissions.usersRead },
      { to: "/roles", label: "Roles", icon: ShieldCheck, permission: AdminPermissions.rolesRead },
      { to: "/operations", label: "Operations", icon: Activity, permission: AdminPermissions.systemRead },
      { to: "/audit", label: "Audit log", icon: FileClock, permission: AdminPermissions.auditRead },
      ...
      { to: "/api-tester", label: "API tester", icon: TerminalSquare, permission: AdminPermissions.systemRead },
    ],
  },
]
