...
export class MomobaseAdminClient extends SessionClient {
  ...
  resources() {
    ...
    this.providers = { list: o => this.get(`/api/admin/providers${query(o)}`), createAccount: p => this.post('/api/admin/providers/accounts', p), updateConfig: (id, config) => this.patch(endpoint('/api/admin/providers/accounts', id) + '/config', { config }), activate: id => this.patch(endpoint('/api/admin/providers/accounts', id) + '/activate'), deactivate: id => this.patch(endpoint('/api/admin/providers/accounts', id) + '/deactivate'), test: id => this.post(endpoint('/api/admin/providers/accounts', id) + '/test'), balance: id => this.get(endpoint('/api/admin/providers/accounts', id) + '/balance'), activeBalances: o => this.get(`/api/admin/balances/providers${query(o)}`), health: o => this.get(`/api/admin/health/providers${query(o)}`) }
