...
export class MomobaseClient extends SessionClient {
  ...
  payment(kind, p, o = {}) { if (!p.payment_method || !p.account?.account) throw new Error('payment_method and account.account are required'); if (p.country !== undefined && p.country.length !== 2) throw new Error('country must be a 2-letter ISO code'); return this.post(`/api/v1/${kind === 'collection' ? 'collections' : 'disbursements'}`, p, o) }
}
