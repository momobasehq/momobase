...
The refactor keeps the external API and database-backed payment behavior while removing redundant repository interfaces, provider decorator stacks, outbox processing, legacy compatibility paths, framework-only routing, and duplicated provider/configuration machinery.
...
5. Routing prefers an active provider account matching the request country, then a `GLOBAL` account, ordered by route priority.
...
7. Routes connect provider accounts to collection or disbursement traffic by payment method, country, and priority.

A provider is routable only when its account and route are active, its capabilities match the request, and a healthy runtime exists.
...
Provider accounts use either an ISO 3166-1 alpha-2 country code such as `UG`, `KE`, `TZ`, or `RW`, or `GLOBAL` as a fallback. Collection and disbursement requests require a two-letter country code. Phone prefixes are validation inputs, not the primary routing decision.
