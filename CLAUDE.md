...
- `internal/http` — `NewRouter` builds a `*fiber.App`. A global `Use` stack (request context, request ID, structured logging, recovery, helmet, CORS, compression) sits in front of route groups: `/api/v1` (app-token payments), `/api/admin` (JWT + one permission per route), `/webhooks` (body-capped), plus the optional embedded dashboard from `web/dashboard`, served at `DASHBOARD_PATH` when `DASHBOARD_ENABLED` is set. Fiber's own middleware is used rather than reimplemented; `internal/http/middleware` holds only what Fiber has no equivalent for — authentication, `RequirePermission`/`RequireAppScope`, `JSONOnly`, `NoCache`, the slog request logger, and the request-ID length bound.
