...
**The toolchain is pinned.** `go.mod` carries `toolchain go1.26.6` alongside `go 1.25.0`. The `go` line stays at the compatibility floor for applications embedding momobase as a library; the `toolchain` line is what this repository tests and publishes with. Every workflow resolves its Go through `go-version-file: go.mod`.

**cgo is required when using SQLite.** The SQLite driver links through cgo; host applications choosing PostgreSQL or MySQL control their own build settings.

Long-form operational documentation lives at https://github.com/momobasehq/momobasehq.github.io.

## Architecture

Momobase is a Go **library**. The root package is the complete public lifecycle facade; host applications own their process, deployment, and provider selection.
...
**No providers are registered by default.** `momobase.New` rejects an empty registry rather than starting a server that cannot execute a payment. `providers/dummy` is the opt-in reference adapter.
...
- `providers` — the public adapter contract plus adapter helpers (`DoJSON`, `Redact`, configuration accessors, amount/status normalization, references). `providers/dummy` is the deterministic in-tree simulator.
...
**Capabilities name the service and payment method** (`Capability{ServiceType, PaymentMethod}`). Route creation and selection both require the pair, so invalid routes fail during setup rather than during a payment.
