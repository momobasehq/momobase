...
func (p *Provider) payment(
	...
	country, currency, ref :=
		strings.ToUpper(req.Country),
		upper(req.Currency, p.cfg.Currency),
		providers.RandomRef("airtel-")
	...
	raw, err := p.call(
		...
		path,
		country,
		map[string]any{"reference": req.Reference, party: person, "transaction": tx},
	)
	if err != nil {
		return nil, err
	}
	return &providers.ProviderPaymentResponse{
		ProviderReference: providers.First(
			providers.Path(raw, "transaction.id"),
			providers.Path(raw, "data.transaction.id"),
			ref,
		),
		Status:  domain.TxProcessing,
		Message: "Airtel request accepted",
		Raw:     raw,
	}, nil
}
func (p *Provider) QueryTransaction(ctx context.Context, ref, country string) (*providers.ProviderTransactionStatus, error) {
	...
	raw, err := p.call(
		ctx,
		http.MethodGet,
		strings.ReplaceAll(p.cfg.StatusPath, "{id}", ref),
		country,
		nil,
	)
	if err != nil {
	...
	status := providers.First(
		providers.Path(raw, "transaction.status"),
		providers.Path(raw, "data.transaction.status"),
		providers.Path(raw, "status.code"),
		providers.Path(raw, "status"),
	)
...
func (p *Provider) QueryBalance(ctx context.Context, country string) (*providers.ProviderBalance, error) {
	...
	currency := providers.First(
		providers.Path(raw, "data.currency"),
		providers.Path(raw, "currency"),
		p.cfg.Currency,
	)
	value, err := providers.ParseAmountToMinor(
		providers.First(
			providers.Path(raw, "data.balance"),
			providers.Path(raw, "balance"),
			providers.Path(raw, "available_balance"),
		),
		currency,
	)
	if err != nil {
...
func (*Provider) VerifyWebhook(_ context.Context, payload []byte, headers map[string]string) (*providers.ProviderWebhookEvent, error) {
	...
	currency := providers.First(
		providers.Path(body, "transaction.currency"),
		providers.Path(body, "data.transaction.currency"),
		providers.Path(body, "data.currency"),
		providers.Path(body, "currency"),
	)
	amount, err := providers.OptionalAmount(
		providers.First(
			providers.Path(body, "transaction.amount"),
			providers.Path(body, "data.transaction.amount"),
			providers.Path(body, "data.amount"),
			providers.Path(body, "amount"),
		),
		currency,
	)
	if err != nil {
	...
	status := providers.First(
		providers.Path(body, "transaction.status"),
		providers.Path(body, "data.transaction.status"),
		providers.Path(body, "status"),
	)
	return &providers.ProviderWebhookEvent{
		ProviderReference: providers.First(
			providers.Path(body, "transaction.id"),
			providers.Path(body, "data.transaction.id"),
			providers.Path(body, "transaction_id"),
			providers.Path(body, "reference"),
			headers["X-Transaction-Id"],
		),
		Status:    providers.PaymentStatus(status),
		EventType: "airtel.payment.updated",
		ExternalReference: providers.First(
			providers.Path(body, "transaction.reference"),
			providers.Path(body, "data.transaction.reference"),
		...
		Phone: providers.First(
			providers.Path(body, "transaction.msisdn"),
			providers.Path(body, "data.transaction.msisdn"),
			providers.Path(body, "data.msisdn"),
			providers.Path(body, "msisdn"),
		),
		Raw: body,
	}, nil
}
func (p *Provider) credentials() bool {
...
func (p *Provider) call(ctx context.Context, method, path, country string, in any) (map[string]any, error) {
	...
	headers := map[string]string{
		...
		"X-Country":     strings.ToUpper(country),
		"X-Currency":    p.cfg.Currency,
	}
...
func parseConfig(raw providers.ProviderConfig) (Config, error) {
	cfg := Config{
		...
		CollectionPath:      "/merchant/v1/payments/",
		DisbursementPath:    "/standard/v1/disbursements/",
		StatusPath:          "/standard/v1/payments/{id}",
...
func upper(a, fallback string) string {
	return strings.ToUpper(providers.First(a, fallback))
}
