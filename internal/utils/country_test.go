...
func TestCountryNormalization(t *testing.T) {
	...
	// language.ParseRegion is deliberately not trusted on its own: it accepts reserved and
	// grouping regions, and rewrites alpha-3 into alpha-2, so "USA" would be stored as "US".
	...
	// UK is exceptionally reserved for GB and ParseRegion resolves it; storing the caller's
	// own input keeps it out of the canonical rewrite path.
